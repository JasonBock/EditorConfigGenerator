...
namespace EditorConfigGenerator.Core.Styles
{
	public sealed class DotnetStyleQualificationForFieldStyle
		: ModelSeverityStyle<BooleanData, SyntaxNode, ModelNodeInformation<SyntaxNode>, DotnetStyleQualificationForFieldStyle>
	{
		...
		public override string GetSetting()
		{
			if (this.Data.TotalOccurences > 0)
			{
				...
				return $"dotnet_style_qualification_for_field = {value}:{this.Severity.GetDescription()}";
			}
			else
			{
				return string.Empty;
				...
				if(node is MemberAccessExpressionSyntax || node is IdentifierNameSyntax)
				{
					if (model.GetSymbolInfo(node).Symbol is IFieldSymbol fieldSymbol)
					{
						...
						else
						{
							...
							if (!fieldSymbol.IsStatic && classNode != null &&
								model.GetDeclaredSymbol(classNode) == fieldSymbol.ContainingType)
							{

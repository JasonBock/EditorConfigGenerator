...
namespace EditorConfigGenerator.Core.Tests.Styles
{
	...
	public static class DotnetStyleObjectInitializerStyleTests
	{
		...
		public static void CreateWithMoreFalseData()
		{
			...
			var style = new DotnetStyleObjectInitializerStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("dotnet_style_object_initializer = false:error"), nameof(style.GetSetting));
		}
		...
		public static void CreateWithMoreTrueData()
		{
			...
			var style = new DotnetStyleObjectInitializerStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("dotnet_style_object_initializer = true:error"), nameof(style.GetSetting));
		}

		[Test]
		public static void Add()
		{
			var style1 = new DotnetStyleObjectInitializerStyle(new BooleanData(1u, 2u, 3u));
			var style2 = new DotnetStyleObjectInitializerStyle(new BooleanData(10u, 20u, 30u));
		...
		[Test]
		public static void UpdateWithCreationThatUsesInitializer()
		{

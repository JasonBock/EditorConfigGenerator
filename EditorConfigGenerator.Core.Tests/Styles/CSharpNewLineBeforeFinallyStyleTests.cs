	...
	public static class CSharpNewLineBeforeFinallyStyleTests
	{
		...
		public static void CreateWithMoreFalseData()
		{
			...
			Assert.That(style.GetSetting(), Is.EqualTo(
				$"{CSharpNewLineBeforeFinallyStyle.Setting} = false:{style.Severity.GetDescription()}"), nameof(style.GetSetting));
		}
		...
		public static void CreateWithMoreTrueData()
		{
			...
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo(
				$"{CSharpNewLineBeforeFinallyStyle.Setting} = true:{style.Severity.GetDescription()}"), nameof(style.GetSetting));
		}

		[Test]
		public static void Add()
		{
			var style1 = new CSharpNewLineBeforeFinallyStyle(new BooleanData(3u, 1u, 2u));
			var style2 = new CSharpNewLineBeforeFinallyStyle(new BooleanData(30u, 10u, 20u));
...
@"public class Foo
{
	public void Bar()
	{
		...
		[Test]
		public static void UpdateWithNoTriviaBeforeFinally()
		{
			var style = new CSharpNewLineBeforeFinallyStyle(new BooleanData(default, default, default));

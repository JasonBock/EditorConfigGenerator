...
namespace EditorConfigGenerator.Core.Tests.Styles
{
	...
	public static class CSharpNewLineBeforeMembersInObjectInitializersStyleTests
	{
		...
		public static void CreateWithMoreFalseData()
		{
			...
			var style = new CSharpNewLineBeforeMembersInObjectInitializersStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("csharp_new_line_before_members_in_object_initializers = false:error"), nameof(style.GetSetting));
		}
		...
		public static void CreateWithMoreTrueData()
		{
			...
			var style = new CSharpNewLineBeforeMembersInObjectInitializersStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("csharp_new_line_before_members_in_object_initializers = true:error"), nameof(style.GetSetting));
		}

		[Test]
		public static void Add()
		{
			var style1 = new CSharpNewLineBeforeMembersInObjectInitializersStyle(new BooleanData(1u, 2u, 3u));
			var style2 = new CSharpNewLineBeforeMembersInObjectInitializersStyle(new BooleanData(10u, 20u, 30u));

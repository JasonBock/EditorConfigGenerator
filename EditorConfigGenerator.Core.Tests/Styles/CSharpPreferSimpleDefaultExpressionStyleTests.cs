	...
	public static class CSharpPreferSimpleDefaultExpressionStyleTests
	{
		...
		public static void CreateWithMoreFalseData()
		{
			...
			Assert.That(style.GetSetting(), Is.EqualTo(
				$"{CSharpPreferSimpleDefaultExpressionStyle.Setting} = false:{style.Severity.GetDescription()}"), nameof(style.GetSetting));
		}
		...
		public static void CreateWithMoreTrueData()
		{
			...
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo(
				$"{CSharpPreferSimpleDefaultExpressionStyle.Setting} = true:{style.Severity.GetDescription()}"), nameof(style.GetSetting));
		}

		[Test]
		public static void Add()
		{
			var style1 = new CSharpPreferSimpleDefaultExpressionStyle(new BooleanData(3u, 1u, 2u));
			var style2 = new CSharpPreferSimpleDefaultExpressionStyle(new BooleanData(30u, 10u, 20u));

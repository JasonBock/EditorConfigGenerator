...
namespace EditorConfigGenerator.Core.Tests.Styles
{
	...
	public static class DotnetStylePreferInferredTupleNamesStyleTests
	{
		...
		public static void CreateWithMoreFalseData()
		{
			...
			var style = new DotnetStylePreferInferredTupleNamesStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("dotnet_style_prefer_inferred_tuple_names = false:error"), nameof(style.GetSetting));
		}
		...
		public static void CreateWithMoreTrueData()
		{
			...
			var style = new DotnetStylePreferInferredTupleNamesStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("dotnet_style_prefer_inferred_tuple_names = true:error"), nameof(style.GetSetting));
		}

		[Test]
		public static void Add()
		{
			var style1 = new DotnetStylePreferInferredTupleNamesStyle(new BooleanData(1u, 2u, 3u));
			var style2 = new DotnetStylePreferInferredTupleNamesStyle(new BooleanData(10u, 20u, 30u));

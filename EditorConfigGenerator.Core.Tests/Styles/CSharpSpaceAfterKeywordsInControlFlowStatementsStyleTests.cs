...
namespace EditorConfigGenerator.Core.Tests.Styles
{
	...
	public static class CSharpSpaceAfterKeywordsInControlFlowStatementsStyleTests
	{
		...
		public static void CreateWithMoreFalseData()
		{
			...
			var style = new CSharpSpaceAfterKeywordsInControlFlowStatementsStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("csharp_space_after_keywords_in_control_flow_statements = false:error"), nameof(style.GetSetting));
		}
		...
		public static void CreateWithMoreTrueData()
		{
			...
			var style = new CSharpSpaceAfterKeywordsInControlFlowStatementsStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("csharp_space_after_keywords_in_control_flow_statements = true:error"), nameof(style.GetSetting));
		}

		[Test]
		public static void Add()
		{
			var style1 = new CSharpSpaceAfterKeywordsInControlFlowStatementsStyle(new BooleanData(1u, 2u, 3u));
			var style2 = new CSharpSpaceAfterKeywordsInControlFlowStatementsStyle(new BooleanData(10u, 20u, 30u));

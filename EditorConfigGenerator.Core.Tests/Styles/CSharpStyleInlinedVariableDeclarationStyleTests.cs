...
namespace EditorConfigGenerator.Core.Tests.Styles
{
	...
	public static class CSharpStyleInlinedVariableDeclarationStyleTests
	{
		...
		public static void CreateWithMoreFalseData()
		{
			...
			var style = new CSharpStyleInlinedVariableDeclarationStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("csharp_style_inlined_variable_declaration = false:error"), nameof(style.GetSetting));
		}
		...
		public static void CreateWithMoreTrueData()
		{
			...
			var style = new CSharpStyleInlinedVariableDeclarationStyle(data);
			Assert.That(style.Data, Is.SameAs(data), nameof(style.Data));
			Assert.That(style.GetSetting(), Is.EqualTo("csharp_style_inlined_variable_declaration = true:error"), nameof(style.GetSetting));
		}
		...
		public static void UpdateWithInlineDeclaration()
		{
			...
			var statement = SyntaxFactory.ParseStatement("Foo(out int x)")
		...
		public static void UpdateWithNoInlineDeclaration()
		{
			...
			var statement = SyntaxFactory.ParseStatement("Foo(out x)")
				.DescendantNodes().Single(_ => _.Kind() == SyntaxKind.Argument) as ArgumentSyntax;
			var newStyle = style.Update(statement);
		...
		public static void UpdateWithNoOutArgument()
		{
			...
			var statement = SyntaxFactory.ParseStatement("Foo(x)")
				.DescendantNodes().Single(_ => _.Kind() == SyntaxKind.Argument) as ArgumentSyntax;
			var newStyle = style.Update(statement);
		...
		public static void UpdateWithDiagnostics()
		{
			...
			var statement = SyntaxFactory.ParseStatement("Foo(*)")
				.DescendantNodes().Single(_ => _.Kind() == SyntaxKind.Argument) as ArgumentSyntax;
			var newStyle = style.Update(statement);
